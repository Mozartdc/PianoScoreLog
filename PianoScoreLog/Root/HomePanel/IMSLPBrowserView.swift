...
struct IMSLPBrowserView: UIViewRepresentable {
    ...
    /// makeUIView 완료 후 WKWebView 인스턴스를 전달하는 콜백 (뒤로/앞으로 버튼용).
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        // 키보드 등장 시 WKWebView 스크롤뷰가 자동으로 content inset을 조정하면
        // .sheet의 키보드 회피 이동과 겹쳐 좌표가 두 번 튀는 현상이 발생한다.
        // .never 로 설정하면 sheet 레벨에서만 키보드 회피가 이뤄져 자연스럽다.
        webView.scrollView.contentInsetAdjustmentBehavior = .never

        webView.load(URLRequest(url: initialURL))

        // WKWebView 인스턴스를 외부로 전달 (툴바 뒤로/앞으로 버튼용)
        DispatchQueue.main.async { self.onWebViewCreated?(webView) }
