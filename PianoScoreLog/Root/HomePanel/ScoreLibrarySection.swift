...
struct ScoreLibrarySection: View {
    ...
    var body: some View {
        ...
        // fullScreenCover: .sheet와 달리 keyboard avoidance가 없어
        // WKWebView 내부 스크롤만 동작 → 검색창 포커스 시 창이 튀지 않음
        .fullScreenCover(isPresented: $showingIMSLP) {
    ...
    ///
    /// 이전 시트(URLImport, IMSLP fullScreenCover 등)의 dismiss 애니메이션이 끝나기 전에
    /// 바로 새 시트를 표시하면 NavigationBar의 UIVisualEffectView(글래스)가
    /// 좌상단에서 시작하는 시각 버그가 발생한다.
    /// 0.4초 지연으로 dismiss 애니메이션(~0.35초)이 완전히 끝난 뒤 표시한다.
