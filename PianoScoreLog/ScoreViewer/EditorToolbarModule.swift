...
private final class EditorToolbarViewController: UIViewController, UIPopoverPresentationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        // 드로잉 제스처가 시작되면 열려 있는 팝오버를 닫는다.
        // Apple Pencil로 바로 필기를 시작해도 옵션창이 방해하지 않도록 한다.
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(handleCanvasDrawingBegan),
            name: .scoreCanvasDrawingGestureBegan,
            object: nil
        )
    }
    ...
    @objc private func handleCanvasDrawingBegan() {
    ...
    private func refreshUI() {
        ...
        for (tool, item) in toolItems {
            item.isEnabled = !unimplementedTools.contains(tool)
            if tool == state.activeDrawingTool {
                // 활성 펜류: 현재 색상 반영 / 그 외(text, eraser, sticker): systemBlue
                item.tintColor = colorTools.contains(tool)
                    ? UIColor(state.selectedDrawingColor)
                    : .systemBlue
            } else {
                item.tintColor = nil
            }
